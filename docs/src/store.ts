	...
	actions: {
		fetchStats: async ({ commit }) => {
			...
			const [fetchedDownloads, fetchedStars, fetchedContributors] = await Promise.all([
				fetch('https://api.npmjs.org/downloads/range/2023-03-14:2100-08-21/discord-rankup').then(toJSON, noop),
				fetch('https://api.github.com/repos/x404dev/discord-rankup').then(toJSON, noop),
				fetch('https://api.github.com/repos/x404/discord-rankup/stats/contributors').then(toJSON, noop),
			]);
